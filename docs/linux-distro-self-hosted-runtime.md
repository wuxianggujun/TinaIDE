...
- `AlpineMirrorManager`：保存 Alpine 镜像偏好；安装前排序 rootfs 下载候选，安装后维护 guest 的 `/etc/apk/repositories`。
...
- **rootfs 下载镜像**：作用于宿主下载 Alpine/Ubuntu rootfs 压缩包。候选来自内置 manifest 的 `mirrors`；Alpine 用户偏好会被排到首位，SHA-256 校验始终以官方 artifact 清单为准。
- **Alpine APK 软件源**：作用于已安装 guest 内的 `/etc/apk/repositories`。切换后必须真实执行 `apk update`；失败时恢复原文件，不保留半生效状态。
- 两条链路职责独立、不能互相替代，但 Alpine 使用同一个用户镜像偏好协调体验：未安装时先影响 rootfs 下载，安装完成后写入 APK 软件源。

## 安装流程

1. 用户在设置页或工作区安装页显式请求 Linux 环境。
2. 安装入口加载远程或缓存 catalog，并创建 `SelfHostedLinuxDistroRuntime`。
3. `LinuxDistroInstaller` 先尝试用户选择的 Alpine 镜像，再尝试官方 URL 与 manifest 中匹配的回落镜像。
4. `LinuxDistroManager` 下载 rootfs、校验 SHA-256，并解包到 `linux-distro/installed-rootfs`。
5. Alpine 安装在 bootstrap 前按用户偏好写入 `/etc/apk/repositories`，避免首次 `apk update` 仍访问官方源。
6. `LinuxDistroRootfsBootstrapper` 进入新 rootfs，补齐 bash、curl、tar、xz、file、ca-certificates 等基础命令。
7. `LinuxDistroRootfsProfileMapper` 写入 `RootfsProfileStore`，并设置为活动 profile。
8. 工作区安装页继续安装可选的 PRoot guest toolchain、Android sysroot 和 native toolchain。
...
- 内置 asset 镜像规则、下载候选排序、APK repositories 生成和镜像检测均有测试覆盖。
