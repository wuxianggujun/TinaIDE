...
> 文档状态：**历史参考**
> 记录时间：2026-07-16（阶段验收口径，未随后续版本刷新）
> 目标：把“插件教程是否真的可用”变成可复查的验收项，避免后续再次回落到普通新建项目、普通 C/C++ 运行链路或旧命令模型。

本文是某一轮教程验收的清单快照。其中的模板名称、课程列表、测试类名和期望文案可能已被后续改动
覆盖；重新走查前请先对照当前 starter 注册表、帮助资产和测试目录确认清单项仍然成立。
当前实现口径见 [README.md](README.md) 与 [Plugin-Authoring-Tutorial.md](Plugin-Authoring-Tutorial.md)。
